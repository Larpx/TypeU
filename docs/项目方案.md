...
- 启动时输入教师工号、姓名，并选择"立即开始考试"或"仅巡视"；一键启动监听
...
- 开考/暂停/恢复/结束联动 TimeSyncService 广播考试状态与剩余时长，作为学生端倒计时基准
...
- 连接后 Hello 握手上报设备指纹；未开考可联网练习（上报状态供巡视，不计成绩、不锁定）且不强制登录
- **仅开考后**允许登录（学号+姓名）；同考试会话内同设备已有登录记录则自动登录
- 登录成功后禁止自行退出，直至教师结束考试或单独允许登出；教师结束考试后允许学生自行选择是否登出以清除本机个人信息
- 按 MaxAttempts 完成 1–N 次测验，每次成绩均回传；交齐后按配置决定是否自由练习
- 打字界面：原文/输入高亮、仪表盘；定期上报速度与是否最小化（联网练习与考试均上报，单机不上报）
- 状态栏显示联网模式（单机/连接中/在线未登录/联网练习/考试已登录/可登出）
